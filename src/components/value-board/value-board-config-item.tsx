...
import { NpmConfigInput } from "../common/npm-config-input";
  ...
  return (
    <Box mt={3}>
      <NpmConfigInput
        ...
        onChangeRange={(range) => onChange({ ...config, range })}
        onChangePackageName={(packageName) => onChange({ ...config, packageName })}
      />
    </Box>
  );
};
